...
fn test_from_object() {
    let node =
        IpldNode::from_object(IpldValue::String("".into()), multihash::Code::Sha2_256).unwrap();
    assert_eq!(
        &node.cid().to_string(),
...
fn test_objects() {
    ...
    if let IpldValue::Map(map) = obj {
        for (key, value) in map {
            ...
            let cbor_file_name = format!("{}{}.core", TEST_OBJ_ROOT, key);
...
fn test_canonical_struct_encoding() {
    ...
    impl<'b> decode::Decode<'b> for Foo {
        fn decode(d: &mut Decoder<'b>) -> Result<Self, decode::Error> {
            ...
            Ok(Foo {
                zebra: d.str()?.to_owned(),
                dog: d.i32()?,
                cats: d.f64()?,
                whale: d.str()?.to_owned(),
                cat: d.bool()?,
            })
        }
    }
...
fn test_resolved_val_is_jsonable() {
    ...
    if let (Either::Right(obj), _) = node.resolve(&["foo"]).unwrap() {
        assert_eq!(serde_json::to_string(&obj).unwrap(), r#"{"bar":1,"baz":2}"#);
    } else {
        unreachable!()
    }
}
